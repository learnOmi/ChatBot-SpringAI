...
public class AgentTools {
    ...
    @Tool(description = "从知识库中检索与用户问题相关的信息")
    ...
    private String fallbackWeather(String city) {
        return String.format("%s当前天气：晴，温度22°C，湿度40%%（模拟数据）", city);
    }

    // ========== 网络搜索（Brave Search）使用 OkHttp ==========
    @Tool(description = "在互联网上搜索信息。当需要查找最新资讯、百科知识或任何实时信息时调用。")
    ...
    private String fallbackWebSearch(String query) {
        return String.format("关于'%s'的搜索结果：1. 相关介绍... 2. 最新动态...（模拟数据）", query);
    }
}